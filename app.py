...
# Page config
st.set_page_config(
    page_title=\"Binance USDT WebSocket Tracker\",
    page_icon=\"🔗\",
    layout=\"wide\"
)
...
async def websocket_connection():
    \"\"\"Pure WebSocket connection - NO REST API\"\"\"
    ...
    try:
            ...
            while not st.session_state.stop_websocket:
                try:
                    ...
                    if isinstance(data, list):
                        ...
                        add_log(f\"WebSocket streamed {len(processed_data)} USDT pairs\")
                    ...
                    add_log(\"Invalid JSON from WebSocket\")
                    continue
        ...
        st.session_state.ws_connected = False
        st.session_state.ws_status = \"Disconnected\"
        add_log(\"WebSocket disconnected\")
...
if st.session_state.ws_data:
    ...
    st.success(f\"Streaming {len(st.session_state.ws_data)} USDT pairs via WebSocket\")
else:
...
st.markdown(\"---\")
st.markdown(\"**NO REST API - Pure WebSocket Implementation**\")
